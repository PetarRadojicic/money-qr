...
type EditCategoryModalProps = {
  visible: boolean;
  onClose: () => void;
  onSave: (data: { name: string; icon: IconName; color: string }) => void;
  onDelete?: () => void;
  categoryKey: TranslationKey | string;
  initialName: string;
...
const EditCategoryModal = ({
  ...
  const handleSave = () => {
    if (categoryName.trim()) {
      onSave({ name: categoryName.trim(), icon: selectedIcon, color: selectedColor });
      onClose();
    }
  };
  ...
  const modalTitle = t("editCategory");
        ...
        >
          ...
          <View className="px-6 pb-6">
            <View className="flex-row items-center justify-between mb-6">
              <Text className="text-2xl font-bold text-slate-900 dark:text-white" numberOfLines={1} style={{ flex: 1 }}>
                {modalTitle}
              </Text>
              {/* Preview of selected category */}
              <View 
                className="rounded-2xl p-3 ml-3"
          ...
          {/* Footer Actions */}
          <View className="p-6 pt-4 border-t border-slate-200 dark:border-slate-800">
            {/* Primary Actions */}
            ...
            {onDelete && (
              <Pressable
                className="bg-red-50 dark:bg-red-900/20 rounded-2xl py-3.5 items-center active:scale-95 border border-red-200 dark:border-red-800"
